...
def parse_expression(expr_str: str) -> Expr:
    ...
    # Look for | (or)
    # FIXED: When scanning right-to-left, ')' increases depth, '(' decreases depth
    depth = 0
    for i in range(len(expr_str) - 1, -1, -1):
        if expr_str[i] == ')':  # FIXED: was '('
            depth += 1
        elif expr_str[i] == '(':  # FIXED: was ')'
            depth -= 1
...
def main():
    ...
    print(f"{Colors.BOLD}{Colors.BLUE}Propositional Logic Autograder (FIXED VERSION){Colors.END}")
